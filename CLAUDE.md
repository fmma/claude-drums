...
2. **Parsing** (`parse_file` → `parse_preamble` + `parse_pattern_block`) — Extracts title, BPM, arrangement (list of lists of pattern names), and pattern dicts with tracks (instrument + step string).
...
**Pattern dict**: `{"name": "VERSE", "tracks": [{"instrument": "HH", "steps": "x-x-x-x-"}], "steps_per_bar": 16, "bpm": 120}`
...
- Step chars: `x`/`o` = normal (vel 90), `X`/`O` = accent (vel 110), `.`/`g` = ghost (vel 30), `-` = rest
...
Preamble has title, BPM, arrangement lines. Drums blocks have a name line then `steps INSTRUMENT` lines. Track length sets grid resolution (16 = 16th notes, 12 = triplets, 8 = 8th notes).
...
Macros are case-insensitive, canonical form is lowercase. They are inline text substitutions — `[pat o-]` becomes `o-o-o-o-...`, `[init 16]` becomes `----------------`. Block macros `[norm]` and `[zoom N]` apply post-processing to all tracks. `[linear A B]` is a standalone line that expands to multiple track lines. All macros are consumed (written back to file).
